...
정규식·치환 규칙·가중치는 전부 이 저장소에서 직접 작성했다. 다만 **분류 체계와
ID 번호**(10대 카테고리, A-1 · C-8b · D-8a …)는 Humanize KR 의 한국어 AI 티
taxonomy 를 따랐고, `evidence` 필드의 실측 배수도 거기서 인용했다. 추적 가능하게
하려고 번호를 그대로 둔 것이다 — 원본에 없는 자체 추가는 50번대를 쓴다.
